...
module "add_bastion_host" {
  count = local.config.createBastionHost.enabled ? 1 : 0

  source            = "./modules/bastionHost"
  requestId         = local.requestId
  vpc_id            = local.config.createBastionHost.vpcID
  private_subnet_id = local.config.createBastionHost.privateSubnetID
}
...
data "aws_iam_policy_document" "core_role_assume_policy" {
  ...
  dynamic "statement" {
    for_each = local.config.createBastionHost.enabled ? [1] : []
    content {
      actions = ["sts:AssumeRole"]
      principals {
        type        = "AWS"
        identifiers = [module.add_bastion_host[0].bastion_role_arn]
      }
    }
  }
}
...
resource "aws_iam_role_policy_attachment" "attach_bastion_policy_0" {
  count = local.config.createBastionHost.enabled ? length(local.core_roles) : 0

  role       = aws_iam_role.core_role[count.index].name
  policy_arn = aws_iam_policy.onehouse_core_role_bastion_policy[0].arn
}
...
resource "aws_iam_policy" "onehouse_core_role_bastion_policy" {
  count = local.config.createBastionHost.enabled ? 1 : 0
  name  = "onehouse-core-role-bastion-policy-${local.truncated_request_id}"
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Resource = [
          module.add_bastion_host[0].bastion_host_arn,
          module.add_bastion_host[0].session_document_arn
        ]
        Condition = {
          BoolIfExists = {
      ...
      },
      {
        ...
        Effect   = "Allow"
        Resource = module.add_bastion_host[0].session_document_arn
      }
    ]
  })
}
...
resource "aws_s3_object" "latest_config_yaml" {
  ...
  depends_on = [
    ...
    aws_iam_policy.onehouse_core_role_bastion_policy,
    module.s3_core_role_access_policy,
    module.s3_node_role_access_policy,
    module.s3_data_load_access_policy,
    module.aggregate_node_role_policies,
